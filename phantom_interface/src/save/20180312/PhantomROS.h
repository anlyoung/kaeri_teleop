...
class PhantomROS {
    ...
    void mesh_update_callback(const visualization_msgs::Marker::ConstPtr& msg) {
		    ...
		    for (int i = 0; i < size; i++){
		        state->mesh_buffer.push_back(msg->points[i].x/state->scale);
		        state->mesh_buffer.push_back(msg->points[i].y/state->scale);
		        state->mesh_buffer.push_back(msg->points[i].z/state->scale);
		    }
