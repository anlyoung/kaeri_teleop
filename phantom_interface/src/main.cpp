...
void HLCALLBACK calibrationCallback(HLenum event, HLuint object, HLenum thread, HLcache *cache, void *userdata);
/*******************************************************************************
...
void initHL()
{
    ...
    hlEnable(HL_PROXY_RESOLUTION);
		...
		hlAddEventCallback(HL_EVENT_CALIBRATION_UPDATE, HL_OBJECT_ANY, HL_CLIENT_THREAD, &calibrationCallback, NULL);
		hlAddEventCallback(HL_EVENT_CALIBRATION_INPUT, HL_OBJECT_ANY, HL_CLIENT_THREAD, &calibrationCallback, NULL);
}

void HLCALLBACK hlTouchCB(HLenum event, HLuint object, HLenum thread, HLcache *cache, void *userdata){
	ROS_INFO("[phantom]mesh touched");	
	state.touched = true;
	//touchedCloth = true;
...
void HLCALLBACK hlUnTouchCB(HLenum event, HLuint object, HLenum thread, HLcache *cache, void *userdata){
	ROS_INFO("[phantom]mesh untouched");
	state.touched = false;
	//touchedCloth = false;
	//movingOnCloth = false;
}
...
void HLCALLBACK hlMotionCB(HLenum event, HLuint object, HLenum thread, HLcache *cache, void *userdata){
	ROS_INFO("[phantom]moving on mesh surface");
	//movingOnCloth = true;
	//hlGetDoublev(HL_PROXY_POSITION, proxyPosition);
	//massTouched = getClosestMass(proxyPosition);
}
...
void HLCALLBACK calibrationCallback(HLenum event, HLuint object, HLenum thread, HLcache *cache, void *userdata){
	if (event == HL_EVENT_CALIBRATION_UPDATE){
		ROS_INFO("[phantom]Device requires calibration update...");
		hlUpdateCalibration();
	}
	else if (event == HL_EVENT_CALIBRATION_INPUT){
		ROS_INFO("[phantom]Device requires calibration input...");
	}
}
