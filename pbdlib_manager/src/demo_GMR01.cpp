							...
							unsigned int dimension)
{
	...
	for (int j = 0; j < GMR_points.n_cols; ++j)
	{
		...
		top_vertices(1, j) = (GMR_points(dimension - 1, j) +
							  sqrt(GMR_sigma[j](dimension - 1, dimension - 1))) *
							 scale_y;
...
int main(int argc, char **argv)
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (!gui_state.is_drawing_demonstration && !gui_state.is_parameters_dialog_displayed)
		{
			...
			if (nBtnState == 2)
				//ImGui::IsMouseClicked(GLFW_MOUSE_BUTTON_1))
			{
				double mouse_x, mouse_y;
				//glfwGetCursorPos(window, &mouse_x, &mouse_y);
				mouse_x = (double) fPhantomX * 100;
				mouse_y = (double) fPhantomY * 100;
				...
				//vec coords = ui2fb({ 100*(int)fPhantomX, 100*(int)fPhantomY }, window_size, viewport_demos);
				//current_trajectory.push_back(coords);
			}
		}
		else if (gui_state.is_drawing_demonstration)
		{
			...
			std::cout << "Mouse Values : (" << mouse_x << "," << mouse_y << std::endl;
			//glfwGetCursorPos(window, &mouse_x, &mouse_y);
