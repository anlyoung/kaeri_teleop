...
//#include "vr_interface_msgs/FloatArray.h"
...
void extractPC(const sensor_msgs::PointCloud2ConstPtr& msg) {
  ...
  else
  {
	...
	std::cerr << "PointClousd representing the cylindrical component: " << cloud_cylinder->points.size () << " data points." << std::endl;
    is_cylinder_found = true;
  }
}
...
//        if (!isnan(cloud.points[x].x) && cloud.points[x].z <2 && cloud.points[x].z >.5f && x%30==n) {
   ...
   // std::cout << "COUNT: " << count <<std::endl;
    ...
    float rSqr;
    Vector3f C(0.0,0.0,0.0);
    Vector3f W(0.0,0.0,0.0);
    float error = 0.0;
...
{
    is_cylinder_found = false;
    ros::init (argc,argv, "PointCloudExtraction");

    ros::NodeHandle n;
    ros::Rate loop_rate(30); // frequency of operation

    // subscribe
    ros::Subscriber sub = n.subscribe ("/kinect/depth_registered/points", 1, callback);
    ros::Publisher pub = n.advertise<std_msgs::Float32MultiArray>("/VR_PointCloud",1);
    ...
    while (ros::ok()) {
        ...
        {
            //std::cout<<"Shutdown "<<cylinder_Pose.orientation.x<<std::endl;
            sub.shutdown();
        }
