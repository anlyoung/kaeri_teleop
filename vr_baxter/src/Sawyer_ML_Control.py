...
def testCallback(msg):
    rt = rospy.Rate(30)
    count = 0
    while (count<len(msg.poses)):
        print (msg.poses[count].position)
        ik_sawyer("right",False,msg.poses[count].position,msg.poses[count].orientation)
        count+=1
        rt.sleep()
...
def main():
    msg = PoseArray()
    msg.header.frame_id = "test_posearray"
    ...
    rospy.Subscriber('/pbdlib_trajgmm_states',PoseArray,testCallback,queue_size = 1)
    rospy.spin()
